...
object MonitorOneStepConvertRateAnalyze {
  ...
  def main(args: Array[String]): Unit = {
    ...
    /**
      ...
      */
     val carTracker =rowRDDByDateRange.map(elem=>{(elem.getString(3),elem)}).groupByKey().map(elem=>{
     //生成按时间排序的卡扣字符串0003, 0006, 0002, 0007, 0005, 0007, 0002
       val str:StringBuffer=new StringBuffer();
      elem._2.toList.sortWith((r1,r2)=>{if (DateUtils.after(r1.getString(4), r2.getString(4))) false else true})
        .foreach(elem=>{str.append(","+elem.getString(4))})
      str.substring(1)
    })

    val  split = roadFlowBroadcast.value.split(",")
    //indexOf 从哪个位置开始查找//indexOf 从哪个位置开始查找

    val index = 0
    //这辆车有多少次匹配到这个卡扣切片的次数
    val count = 0L
    while (carTracker.indexOf())
sc.stop()
  ...
  /**
    ...
    * @param taskParam
    */
  def getCameraRDDByDateRange(sqlContext: SQLContext, taskParam: JSONObject) ={
...
def matchRowSplit(carTracker:RDD[Row],roadFlowBroadcast:Broadcast[String],taskParam:JSONObject): Unit ={
